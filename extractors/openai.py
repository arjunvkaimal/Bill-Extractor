"""
OpenAI extractor — uses the openai SDK to call GPT-4o vision API.
"""
...
def extract_bill(image_path: str) -> dict:
    """
    Extract structured data from a bill image using GPT-4o.
    ...
    try:
        ...
        logger.error("OpenAI returned non-JSON for %s: %s", image_path, e)
        result["raw_model_response"] = f"JSON_PARSE_ERROR: {result['raw_model_response']}"
    except Exception as e:
        logger.error("OpenAI extraction failed for %s: %s", image_path, e)
